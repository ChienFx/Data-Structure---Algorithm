...
BTreeNode::~BTreeNode()
{
	...
	{
		for (int i = 0; i < n;i++)
			delete C[i];// gọi hàm hủy của các Node con
		delete[]C;
	}
	
	if (keys)
		delete[]keys;
	cout << "delete BTreeNode" << endl;
}
...
BTreeNode* BTreeNode::search(Data k, int &index)
{
	int i = 0;
	while (i<n && k > keys[i])
		i++;
...
void BTreeNode::insertNonNull(Data k)
{
	int i = n - 1;
	if (leaf)
	{
...
Data BTreeNode::getPredessor(int idx)
{
...
Data BTreeNode::getSuccessor(int idx)
{
